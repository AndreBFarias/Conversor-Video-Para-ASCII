...
def iniciar_conversao_imagem(image_path, output_dir, config):
    ...
    if not os.path.exists(image_path):
        raise FileNotFoundError(f"Erro: '{image_path}' não encontrado.")
    ...
    if frame_colorido is None:
        raise IOError(f"Erro: Não foi possível ler a imagem '{image_path}'.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Executor de Conversão de Imagem para ASCII (CLI)")
    ...
    try:
        print(f"Iniciando conversão de imagem (CLI) para: {args.image}")
